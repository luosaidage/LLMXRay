...
- **完全本地运行**：所有检测逻辑均在您的本地环境执行，不依赖任何第三方云端存储。
- **不存储 API Key**：工具不会以任何形式（日志、数据库、临时文件等）保留或记录您的 API Key。
- **无泄露风险**：API Key 仅用于向您指定的中转地址发起探测请求，绝不会上传到任何非目标服务器。
- **透明可审计**：代码完全开源，您可以随时审计网络请求逻辑。
